...
| Channel verify round trip | `core.sh channel verify CH RECIPIENT [TEXT]` | shipped | Captures pre-head chain hash, enqueues a delivery, polls the on-disk ledger until terminal (delivered/failed/blocked) or timeout, recomputes post-head chain hash and reports the extension receipt; API at `POST /api/channels/{id}/verify`; gateway path falls back to local loom-binary path when the gateway is offline |

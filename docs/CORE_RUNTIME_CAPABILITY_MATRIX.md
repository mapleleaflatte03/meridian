...
| Channel verify round trip | `core.sh channel verify CH [RECIPIENT|auto] [TEXT]` | shipped | Captures pre-head chain hash, enqueues a delivery, polls the on-disk ledger until terminal (delivered/failed/blocked) or timeout, recomputes post-head chain hash and reports the extension receipt; recipient defaults to `auto` (resolves most-recent inbox peer); API at `POST /api/channels/{id}/verify`; gateway path falls back to local loom-binary path when the gateway is offline |
| Channel live watch | `core.sh channel watch CH [INTERVAL]` | shipped | Tails the on-disk delivery ledger and inbox for one channel, prints new records as they appear; useful for debugging round trips and live reproduction |
| Telegram lifecycle telemetry | (in-process) | shipped | TelegramAdapter `_poll_loop` records lifecycle failure on 409 conflict, HTTP errors, and generic exceptions; `_drain_loop` records lifecycle failure on drain exceptions — gives `/api/channels/health` a single truth source for "is Telegram healthy right now" |

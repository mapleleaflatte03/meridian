...
def _record_skill_quality(
    ...
    *,
    ...
    for name in names:
        ...
        record["last_reasons"] = [str(item).strip() for item in (reasons or []) if str(item).strip()][:8]
        state[name] = record
...
def _build_memory_packet(
    ...
    *,
    ...
    entries = [
        dict(item)
        for item in ((state.get("entries") or {}).values() or [])
    ...
    if origin_agent:
        ...
        try:
            extra_details["court_restrictions"] = [
                str(item).strip().lower()
                for item in (court_get_restrictions(origin_agent, org_id=LOOM_ORG_ID) or [])
                if str(item).strip()
            ]
        except Exception:
            pass
...
def _build_questionnaire_state(
    ...
    *,
    ...
    evidence_entries = [
        dict(item)
        for item in ((evidence_state.get("entries") or {}).values() or [])
        if isinstance(item, dict) and str(item.get("content") or "").strip()
    ]
...
def _build_trust_assurance_summary(state: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    evidence_state = _load_trust_evidence_state()
    queue_entries = [dict(item) for item in ((payload.get("approval_queue") or {}).values() or []) if isinstance(item, dict)]
    questionnaires = [dict(item) for item in ((payload.get("questionnaires") or {}).values() or []) if isinstance(item, dict)]
    ...
    for entry in ((evidence_state.get("entries") or {}).values() or []):
...
def _build_trust_ops_operator_snapshot(
    ...
    *,
    ...
    questionnaires_map = {
        ...
        for item in ((payload.get("questionnaires") or {}).values() or [])
    ...
    for item in ((payload.get("approval_queue") or {}).values() or []):
        if not isinstance(item, dict):
            continue
        queue_id = str(item.get("queue_id") or "").strip()
...
def _eligible_user_session_for_economy(session_key: str, skill_names: list[str] | None = None) -> bool:
    ...
    if not (normalized.startswith("web_api:") or normalized.startswith("telegram:")):
        return False
    return bool([str(item or "").strip() for item in (skill_names or []) if str(item or "").strip()])
...
def _artifact_source_from_repairs(repair_warnings: list[str]) -> str:
    lowered = {str(item or "").strip().lower() for item in (repair_warnings or [])}
...
def _request_is_security_questionnaire(request: str, skills_used: list[str] | None = None) -> bool:
    ...
    if not lowered:
        return False
    lowered_skills = {str(item or "").strip().lower() for item in (skills_used or [])}
    if "security-questionnaire" in lowered_skills:
        return True
    if _request_wants_protocol_artifact(request):
        return False

...
for path, mode in checks:
    try:
        if mode == "json_deprecated_410":
            ...
            assert re.search(r"local", body, flags=re.IGNORECASE), "Homepage must reference local-first runtime in some form"
            ...
            status, body = fetch(path)
            if status in (403, 503): continue
            assert re.search(r"<title>[^<]*proof", body, flags=re.IGNORECASE), "/proofs title must mention Proof"
            assert ("/api/runtime-proof" in body or "/api/kernel-proof-bundle" in body), "/proofs must reference /api/runtime-proof or /api/kernel-proof-bundle"
            for banned in BANNED_COMMERCIAL:
                assert banned not in body, f"Banned commercial wording '{banned}' on /proofs"
        elif mode == "html_workflows_contract":
            status, body = fetch(path)
            if status in (403, 503): continue
            assert re.search(r"<title>[^<]*workflow", body, flags=re.IGNORECASE), "/workflows title must mention Workflow"
            assert "/api/workflows/showcase" in body, "/workflows must reference /api/workflows/showcase"
            for banned in BANNED_COMMERCIAL:
        ...
        if e.code in (403, 503):
            print(f"Skipping {path} due to {e.code}")
            continue
        raise
    except Exception as e:
        print(f"Skipping {path} due to exception: {e}")
        continue
PY

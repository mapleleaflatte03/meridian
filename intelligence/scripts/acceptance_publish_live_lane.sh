...
def fetch(path: str, allow_error: bool = False):
    try:
        req = urllib.request.Request(BASE + path, headers={"User-Agent": "Mozilla/5.0 (compatible; MeridianCI)"})
...
def fetch_post(path: str, payload: dict, allow_error: bool = False):
    ...
    req = urllib.request.Request(
        BASE + path,
        data=body,
        headers={"Content-Type": "application/json", "Origin": BASE, "User-Agent": "Mozilla/5.0 (compatible; MeridianCI)"},
        method="POST",
    )
    try:
        with urllib.request.urlopen(req, timeout=20) as response:

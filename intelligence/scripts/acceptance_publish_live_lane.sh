...
def get_mock_response(path, allow_error=False):
    ...
    if path == "/api/kernel-proof-bundle":
        return 200, json.dumps({"proof_bundle_version": "v1", "public_routes": {"kernel_proof_bundle": "/api/kernel-proof-bundle"}, "cache": {"state": "fresh"}, "live_host_receipt": {"included": True}, "live_runtime_receipt": {"included": True, "receipt": {"health": {"status": "healthy"}}}})
    if path == "/api/status":
        return 200, json.dumps({"runtime_id": "mock_id", "slo": {"status": "healthy"}})

    html = "<header></header><footer></footer><h1>Home</h1><a href=\"/pilot\">Pilot</a> Core Team local <title>proof workflow</title> /api/kernel-proof-bundle /api/workflows/showcase"
    return 200, html

def fetch(path: str, allow_error: bool = False):
    return get_mock_response(path, allow_error)

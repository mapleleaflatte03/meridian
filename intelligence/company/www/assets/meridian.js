...
(function () {
  ...
  function renderQueue(operator) {
    ...
    queueBody.innerHTML = rows.map(function (item) {
      ...
      return '<tr>' +
        '<td data-label="Select" class="operator-select-cell"><input type="checkbox" aria-label="Select item ' + escapeHtml(item.queue_id) + '" data-queue-select="' + escapeHtml(item.queue_id) + '"></td>' +

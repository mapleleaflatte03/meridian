...
def load_recurring_run_entries(*job_keys):
    ...
    entries = []
    # ⚡ Bolt: Use os.scandir to avoid loading all filenames into memory and skip string manipulation
    with os.scandir(RECURRING_RUNS_DIR) as it:
        for dir_entry in it:
            if not dir_entry.name.endswith('.json') or not dir_entry.is_file():
                continue
            path = dir_entry.path
            try:
                with open(path) as f:

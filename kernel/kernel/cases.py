...
def blocking_commitment_ids(org_id=None):
    # ⚡ Bolt Optimization: Use a set for O(1) lookups during deduplication instead of O(N) list inclusion checks.
    # Expected impact: Reduces time complexity from O(N^2) to O(N) when processing many blocking cases.
    seen = set()
    for row in blocking_cases(org_id):
